...
# Local and APGAP Cloud BLAST Database Configuration

The pipeline stages the BLAST database files into the BLAST task rather than relying on a container bind mount. The selected profile determines the database location.

## Laptop or workstation

Build the database once as described above. Standard Docker and Apptainer runs use:

```text
FLU_DB/fluA_db/fluA_db.*
```
...
The `apgap` profile points to the database uploaded through Seqera Data Explorer:

```text
gs://infl-seqera-output-085c/fluA_db/fluA_db.*
```

Launch the pipeline with the combined profiles:

```text
docker,apgap
```

The `apgap` profile also applies the Google Batch MultiQC permission workaround without changing ordinary laptop behavior.

The cloud database folder must contain all files generated by `makeblastdb`, including at minimum:
